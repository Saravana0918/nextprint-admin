...
(function(){
  ...
  // form submit: capture preview with html2canvas and POST
...
<script>
(function(){
  // Mobile overlay builder — only one overlay; syncs to #np-name and #np-num
  function mobileOverlaySetup() {
    if (window.innerWidth > 767) return; // only mobile
    const stage = document.getElementById('np-stage');
    if (!stage) return;
    if (document.querySelector('.np-mobile-controls')) return; // already created
    ...
    // keep stage visible on keyboard using visualViewport
  ...
  window.addEventListener('orientationchange', ()=> setTimeout(mobileOverlaySetup, 200));
})();
</script>
