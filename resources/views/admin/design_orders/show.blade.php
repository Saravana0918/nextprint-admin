...
<div class="container py-4">
  ...
  <div class="row g-4">
    <div class="col-md-6">
      <div class="card mb-3">
        <div class="card-body">
          <h5 class="card-title mb-2">{{ $order->product_name ?? ('Product #' . ($order->product_id ?? '—')) }}</h5>
          ...
          <p class="mb-1">
            ...
            {{ $order->customer_number ?? ($order->number ?? '—') }}
          </p>
          ...
          <pre style="white-space:pre-wrap; max-height:180px; overflow:auto; background:#f8f9fa; padding:8px; border-radius:6px;">{{ json_encode(json_decode($order->payload ?? '{}'), JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES) }}</pre>
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-body">
          ...
          @php
            $preview = $order->preview_src ?? $order->preview_image ?? null;
            $previewUrl = $preview;
                ...
                <tbody>
                  @foreach($players as $p)
                    @php
                      ...
                      $pUrl = $pPreview;
                      if ($pPreview && (Str::startsWith($pPreview, '/storage') || Str::startsWith($pPreview, 'storage'))) {
                        $pUrl = asset($pPreview);
                      }
                    @endphp
                    <tr>
                      ...
                      <td class="text-muted small">{{ \Carbon\Carbon::parse($p->created_at ?? $p->created_at ?? now())->format('d M Y') }}</td>
                    </tr>
                  @endforeach
                </tbody>
              </table>
