...
class ShopifyCartController extends Controller
{
    public function addToCart(Request $request)
    {
        ...
        // 1) If variant not provided -> try DB lookup (if table exists)
        ...
        // 2) If still no variantId -> fetch variants from Shopify Admin API (live)
            ...
            try {
                ...
                if ($shop && $token) {
                    ...
                    if ($resp->successful() && !empty($resp->json('product'))) {
                        ...
                        // try to find by option (e.g. option1 or title)
        ...
        if (empty($shop) || empty($token)) {
            Log::error('designer: shopify credentials missing');
            return back()->withErrors(['shopify' => 'Shopify config missing.']);
        }
        ...
        $payload = [
            'draft_order' => [
                'line_items' => [[
                    ...
                    'properties' => [
                        ['name' => 'Customization', 'value' => json_encode($custom)]
                    ]
                ]],
        ...
        if (empty($draft['invoice_url'])) {
            Log::error('designer: draft_no_invoice', ['body'=>$response->body()]);
            return back()->withErrors(['shopify' => 'Draft created but no invoice URL.']);
        }
