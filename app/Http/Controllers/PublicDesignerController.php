    ...
    public function show(Request $request)
{
    ...
    // try to locate product; eager-load variants and views+areas
    $product = null;

    if ($productId) {
        // first try matching shopify_product_id if looks numeric long id
        if (ctype_digit((string)$productId) && strlen((string)$productId) >= 8) {
            $product = Product::with(['variants','views','views.areas'])
        ...
        if (!$product) {
            ...
            if (ctype_digit((string)$productId)) {
                $product = Product::with(['variants','views','views.areas'])->find((int)$productId);
            }
        }

        // fallback: try find by other columns (name, sku, shopify_product_id)
        if (!$product) {
            $query = Product::with(['variants','views','views.areas']);
            $cols = [];
    ...
    $view = null;
    if ($viewId) {
        $view = \App\Models\ProductView::with('areas')->find($viewId);
    }
    if (!$view) {
    ...
    if (!isset($layoutSlots['name'])) {
        $layoutSlots['name'] = ['id'=>null,'left_pct'=>50,'top_pct'=>25,'width_pct'=>85,'height_pct'=>8,'rotation'=>0];
    }
    if (!isset($layoutSlots['number'])) {
        $layoutSlots['number'] = ['id'=>null,'left_pct'=>50,'top_pct'=>54,'width_pct'=>70,'height_pct'=>12,'rotation'=>0];
    }

    // compute displayPrice robustly
    $displayPrice = null;
    try {
